...
export class LocationService {
  constructor(
    @InjectRepository(Location) private locatinRepository: Repository<Location>,
  ) {}

  create(project: CreateLocationInput): Promise<Location> {
    let proj = this.locatinRepository.create(project);
    return this.locatinRepository.save(proj);
  }
  ...
  async findOne(id: string): Promise<Location> {
    return await this.locatinRepository.findOne({ where: { id } });
  }
